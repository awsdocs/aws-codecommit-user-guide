...
+ [Step 4: Connect to the AWS CodeCommit Console and Clone the Repository](#setting-up-https-unixes-connect-console)
+ [Next Steps](#setting-up-https-unixes-next-step)

## Step 1: Initial Configuration for AWS CodeCommit<a name="setting-up-https-unixes-account"></a>
...
1. Run git config \-\-global \-\-edit to verify the preceding value has been written to `~/.gitconfig`\. If successful, you should see the preceding value \(in addition to values that may already exist in the Git global configuration file\)\. To exit, typically you would type **:q**, and then press Enter\. 
...
## Step 4: Connect to the AWS CodeCommit Console and Clone the Repository<a name="setting-up-https-unixes-connect-console"></a>
